 ...
 * 
 */
package com.git.gdsbuilder.geogig.command.repository.branch;
...
public class CheckoutBranch {
	 ...
	 *            해당 트랜잭선 ID가 존재하지 않는 경우 Command 실행 실패
	 * @param path
	 *            현재 Checkout된 Branch의 충돌이 발생한 객체의 경로
	 * @param version
	 *            "ours" 또는 "theirs" 문자열만 입력 가능
	 *            <p>
	 *            "ours" : 현재 Checkout된 Branch의 객체로 덮어씌움
	 *            <p>
	 *            "theirs" : 다른 Branch의 객체로 현재 현재 Checkout된 Branch의 객체를 덮어씌움
	 * @return Command 실행 성공 - Checkout을 성공한 객체의 경로와 version("ours" 또는 "theirs")
	 *         반환
	 *         <p>
	 *         Command 실행 실패 - error 반환
	 * 
