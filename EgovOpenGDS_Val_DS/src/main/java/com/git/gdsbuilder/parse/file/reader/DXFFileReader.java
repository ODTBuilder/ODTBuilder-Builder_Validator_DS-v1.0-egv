package com.git.gdsbuilder.parse.file.reader;
...
import com.git.gdsbuilder.parse.file.dxf.DXFFileLayerParser;
...
public class DXFFileReader {
	 ...
	 * <p>
	 * DXF는 AutoDAD 도면 파일로 {@link Geometry} 형태가 아닌 Line, Polyline,
	 * Blocks/Insert, Text 등의 Entity로 구성되어 검수 시 {@link Geometry} 공간 연산(Equals,
	 * Intersects, Contains 등)을 사용할 수 없음.
	 * <p>
	 * 따라서 각 레이어의 Entity를 호환 가능한
	 * {@link Point}/{@link LineString}/{@link Polygon} 타입으로 변환 후
	 * {@link DTLayer}로 생성함.
	 * <p>
	 * 검수 영역 레이어는 {@link DTLayerCollection}의 {@link DTLayerList}에 포함시키지 않고
	 * neatLine 변수에 저장함.
	 * 
	 * @param epsg
	 *            좌표계 (ex EPSG:4326)
	 ...
	 * @param neatLineName
	 *            검수 영역 레이어 이름
	 * @return DXF 파일을 변환한 {@link DTLayerCollection}, DXF 파일 내에 레이어가 존재하지 않거나
	 *         올바르지 않은 형태이면 {@code null} 반환
	 * @throws Exception
	 * 
	 * @author DY.Oh
	 */
